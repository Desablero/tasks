...
//=====================================================================================================================>
// 25/35 Напишите функцию unique_letters(str), которая возвращает строку, оставив в ней только уникальные символы,
// т.е. встречающиеся в строке один раз
/*
...
function unique_letters(str) {
    ...
    for (i=0;i<arr.length; i++) {
        let letter = arr[i] // буферим итерируемый элем масс
        arr[i] = '' // скрываем этот элемент с проверки includes (но не удаляем!)
        // Если такого элемента больше нету в массиве - пушим в результат
        if (!arr.includes(letter)) {
            result.push(letter)
        }
        arr[i] = leter // возвращаем value элемента чтобы оставить массив в сохранности
        }
        return result.join(',')
}
 ...
 */
// Пришлось поднапрячься. Но как по мне всё равно велосипед получился...
// Ответ: var str = str.split('');
//   var res = str.filter(function(val, i, str) {
//   return str.lastIndexOf(val) === str.indexOf(val);});
//   return res;}
//====================================================================================================================>
// 26/35 Напишите функцию removeDuplicate(str), которая возвращает строку, очищенную от слов-дупликатов,
...
function removeDuplicate(str) {
        ...
        arr = str.split(',')
        for (i=0;i<arr.length; i++) {
            let word = arr[i] // буферим итерируемый элем масс
            arr[i] = '' // скрываем этот элемент с проверки includes (но не удаляем!)
            // Если такого элемента больше нету в массиве - пушим в результат
...
// #EasyFast. Ответ: for(var i = 0; i < str.length ; i++) {
...
function delete_first_occurrence(str, substr) {
    arr = str.split(', ')
    arr.splice(arr.indexOf(substr),1)
    return arr.join(', ')
...
// ====================================================================================================================>
// 29/35 Напишите функцию decimalToHexString(number), которая преобразовывает десятичные значения в их 
// шестнадцатеричный эквивалент hex. Hex (hexadecimal) — обозначение шестнадцатеричной системы счисления
/*

function decimalToHexString(number) {
    number = parseInt(number,10) // переводим стринг в number
    return number.toString(16) // обратно в строковое значение в 16-й сис. счисл.
}
console.log(decimalToHexString('12312313'));
// #EasyFast. Ответ: return hexString = Number(d).toString(16);
